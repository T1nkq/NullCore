    ...
    Task.Run(() =>
    {
        ...
        private void ConnectRpc(bool isAutoReconnect = false)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(App.DiscordApplicationId))
                {
                    Status = "RPC disabled. | ";
                    if (!isAutoReconnect)
                        Frontend.ShowMessageBox("Discord RPC is disabled because no NullCore Discord application ID is configured.");
                    return;
                }

                _rpcClient = new DRPC.DiscordRpcClient(App.DiscordApplicationId);
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = $"{App.ProjectName} Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "https://raw.githubusercontent.com/T1nkq/NullCore/main/Bloxstrap/Voidstrap.png",
                        LargeImageText = $"{App.ProjectName} Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in {App.ProjectName}{loopText}";
