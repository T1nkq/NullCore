...
namespace Voidstrap.UI.ViewModels.Settings
{
    public class BehaviourViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var settings = new NullCoreRobloxSettingsManager.NullCoreRobloxSettings
            {
                MemoryCleanerIntervalSeconds =
                    SelectedMemoryCleanerInterval?.Seconds ?? 0
            };

            NullCoreRobloxSettingsManager.Save(settings);
        }
        ...
        {
            get => CleanerItems.Contains("AppLogs");
            set
            {
                if (value && !CleanerItems.Contains("AppLogs"))
                {
                    CleanerItems.Add("AppLogs");
                    UpdateCleanerItems();
                }
                else if (!value && CleanerItems.Contains("AppLogs"))
                {
                    CleanerItems.Remove("AppLogs");
                    UpdateCleanerItems();
                }
